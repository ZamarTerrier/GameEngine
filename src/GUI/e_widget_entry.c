...
void EntryWidgetKeyCallback(void* arg,  int key, int scancode, int action, int mods)
{
    ...
    if(key == ENGINE_KEY_LEFT_CONTROL && action == ENGINE_PRESS)
        e_ctrl_press = true;
    else if(key == ENGINE_KEY_LEFT_CONTROL && action == ENGINE_RELEASE){
        e_ctrl_press = false;
    ...
    if(key == ENGINE_KEY_V && action == ENGINE_PRESS)
        e_v_press = true;
    else if(key == ENGINE_KEY_V && action == ENGINE_RELEASE){
        e_v_press = false;
        e_pasted = false;
    }

    if(key == ENGINE_KEY_C && action == ENGINE_PRESS)
        e_c_press = true;
    else if(key == ENGINE_KEY_C && action == ENGINE_RELEASE){
        e_c_press = false;
        e_copied = false;
    }

    if(action == ENGINE_PRESS)
        WidgetConfirmTrigger(e_var_current_entry, ENGINE_WIDGET_TRIGGER_ENTRY_KEY_PRESS_INPUT, key);
    else if(action == ENGINE_REPEAT)
        WidgetConfirmTrigger(e_var_current_entry, ENGINE_WIDGET_TRIGGER_ENTRY_KEY_REPEAT_INPUT, key);
    else if(action == ENGINE_RELEASE)
