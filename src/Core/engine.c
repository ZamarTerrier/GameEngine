...
void EngineInitSystem(int width, int height, const char* name){
    ...
    wManagerSetCharCallback(e_window, EngineCharacterCallback);
    wManagerSetKeyCallback(e_window, EngineKeyCallback);

    e_var_current_entry = NULL;
}

void EngineFixedCursorCenter(){
    wManagerSetCursorPos(e_window, WIDTH / 2, HEIGHT / 2);
}
...
void EngineSetCursorPos(float xpos, float ypos){
    wManagerSetCursorPos(e_window, xpos, ypos);
}

void EngineHideCursor(char state){
    switch(state){
        case 0 :
            wManagerSetInputMode(e_window, ENGINE_CURSOR, ENGINE_CURSOR_DISABLED);
            break;
        case 1 :
            wManagerSetInputMode(e_window, ENGINE_CURSOR, ENGINE_CURSOR_HIDDEN);
            break;
        case 2 :
            wManagerSetInputMode(e_window, ENGINE_CURSOR, ENGINE_CURSOR_NORMAL);
            break;
    }
}

int EngineGetMousePress(int Key){
    int state = wManagerGetMouseButton(e_window, Key);
...
const char *EngineGetClipBoardString(){
    return wManagerGetClipboardString(e_window);
}

void EngineSetClipBoardString(const char *string){
    wManagerSetClipboardString(e_window, string);
}
...
int EngineGetKeyPress(int Key){
    int res = wManagerGetKey(e_window, Key);

    return res;
}

void EngineSetMouseKeyCallback(void *callback){
    wManagerSetMouseButtonCallback(e_window, callback);
}
...
void EngineSetCursorPoscallback(void * callback){
    wManagerSetCursorPosCallback(e_window, callback);
}
...
void EnginereRecreateSwapChain() {
    ...
    wManagerGetFramebufferSize(e_window, &WIDTH, &HEIGHT);
...
void EngineCleanUp(){
    ...
    wManagerDestroyWindow(e_window);
