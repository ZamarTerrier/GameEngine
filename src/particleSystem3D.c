...
void Particle3DDefaultUpdate(ParticleObject3D* particle){
    ...
    DescriptorUpdate(&particle->go.graphObj.blueprints, 0, 0, &mbo, sizeof(mbo));
...
void Particle3DAddDefault(ParticleObject3D* particle, void *render)
{
    ...
    BluePrintAddUniformObject(&particle->go.graphObj.blueprints, nums, sizeof(ModelBuffer3D), VK_SHADER_STAGE_VERTEX_BIT);
